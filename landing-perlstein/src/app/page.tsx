...
export default function PerlsteinLandingPage() {
  return <main className={styles.page}>
    ...
    <section id="content" tabIndex={-1} className={styles.hero}>
      ...
      <div className={styles.heroCopy}><span className={styles.eyebrow}><Sparkles size={14} /> ליווי אישי, מקצועי וברור</span>
        ...
        <div className={styles.actions}><a className={styles.primary} href="http://104.207.131.136:3747/check?source=perlstein">לשאלון ההתאמה <ArrowLeft size={19} /></a><a className={styles.secondary} href="https://wa.me/972533140442"><MessageCircle size={18} /> שיחה ב־WhatsApp</a></div>
    ...
    <section id="why" className={styles.comparison}><span aria-hidden="true" className={styles.ghostNum}>01</span><div className={styles.sectionIntro} data-reveal=""><span>הבדל קטן בגישה. הבדל גדול בהחלטה.</span><h2>אפשר לגשת לבד לבנק.<br />אפשר להגיע מוכנים.</h2></div><div className={styles.compareGrid}>
      <article className={styles.without} data-reveal=""><div className={styles.compareTitle}><X /><h3>לבד מול הבנק</h3></div><ul><li>משווים בעיקר את הריבית שמופיעה בכותרת</li><li>מקבלים החלטות מתוך לחץ של זמן</li><li>מתנהלים מול כמה גורמים בלי תמונה אחת</li><li>מגלים את המשמעות של המסלולים בהמשך</li></ul></article>
      <article className={styles.with} data-reveal=""><div className={styles.compareTitle}><Check /><h3>עם פרלשטיין משכנתאות</h3></div><ul><li>מתחילים מהחיים שלכם ומהיכולת האמיתית</li><li>בונים אסטרטגיה לפני שפונים לבנקים</li><li>מנהלים השוואה ומשא ומתן מסודרים</li><li>מקבלים הסבר וליווי עד לביצוע</li></ul></article>
    </div></section>

    <section id="solutions" className={styles.outcomes}><span aria-hidden="true" className={styles.ghostNum}>02</span><div className={styles.sectionIntro} data-reveal=""><span>לא רשימת שירותים. תוצאה.</span><h2>החלטה גדולה צריכה להרגיש ברורה</h2><p>המטרה היא לא רק לקבל אישור — אלא להבין את הדרך ולבחור בה בביטחון.</p></div><div className={styles.outcomeGrid}>{outcomes.map(({icon: Icon,title,text}, index)=><article key={title} data-reveal=""><small>0{index+1}</small><Icon /><h3>{title}</h3><p>{text}</p></article>)}</div></section>

    <GoalSelector />

    <section id="about" className={styles.about}><span aria-hidden="true" className={styles.ghostNum}>04</span><div className={styles.aboutPhoto} data-reveal=""><Image src="/brands/perlstein/portrait-ai.png" alt="מנחם פרלשטיין, יועץ משכנתאות" fill sizes="(max-width: 800px) 90vw, 440px" /></div><div className={styles.aboutCopy} data-reveal=""><span>נעים להכיר</span><h2>מנחם פרלשטיין.<br />איתכם מול ההחלטה.</h2><p className={styles.leadQuote}>“המטרה שלי היא שתבינו כל החלטה, תרגישו בשליטה ותדעו שיש מי שמנהל איתכם את הדרך.”</p><p>בפרלשטיין משכנתאות כל תיק מתחיל בהקשבה. רק אחרי שמבינים את המשפחה, העסקה והתוכניות קדימה — מתכננים את המימון וניגשים לבנקים.</p><a href="tel:0533140442">לשיחה עם מנחם <ArrowLeft size={18} /></a></div></section>

    <section id="process" className={styles.process}><span aria-hidden="true" className={styles.ghostNum}>05</span><div className={styles.sectionIntro} data-reveal=""><span>מהרעיון ועד לביצוע</span><h2>ארבעה צעדים. דרך אחת מסודרת.</h2></div><div className={styles.steps}>{process.map(([number,title,text])=><article key={number} data-reveal=""><b>{number}</b><div><h3>{title}</h3><p>{text}</p></div></article>)}</div></section>

    <section className={styles.questionnaire}><span aria-hidden="true" className={styles.ghostNum}>06</span><div data-reveal=""><span>הצעד הראשון לוקח כמה דקות</span><h2>מתחילים בשאלון קצר.<br />מגיעים לשיחה מוכנים.</h2><p>השאלון מרכז את הנתונים הראשוניים ועוזר לנו להבין במה להתמקד כבר מהשיחה הראשונה.</p><a href="http://104.207.131.136:3747/check?source=perlstein">למילוי שאלון הלקוח <ArrowLeft size={20} /></a></div><div className={styles.questionCard} data-reveal=""><Image src="/brands/perlstein/mark.png" alt="" width={48} height={48} /><span>שאלון התאמה אישי</span><div className={styles.progress}><i /></div><small>מטרת המשכנתא</small><b>רכישת נכס</b><small>השלב הבא</small><b>תמונת הכנסות והתחייבויות</b><em>התצוגה להמחשה בלבד</em></div></section>

    <section id="questions" className={styles.faq}><span aria-hidden="true" className={styles.ghostNum}>07</span><div className={styles.sectionIntro} data-reveal=""><span>שאלות נפוצות</span><h2>כדאי לדעת לפני שמתחילים</h2></div><div className={styles.faqList} data-reveal=""><details><summary>מתי כדאי לפנות לייעוץ?</summary><p>ככל שמקדימים, אפשר להבין את מסגרת התקציב והאפשרויות לפני שמתחייבים לעסקה.</p></details><details><summary>כבר יש לי הצעה מהבנק. עדיין כדאי לבדוק?</summary><p>כן. חשוב לבדוק לא רק את הריבית, אלא גם את מבנה המסלולים, הסיכונים וההתאמה לתוכניות שלכם.</p></details><details><summary>מה צריך להכין לשיחה הראשונה?</summary><p>מתחילים בשאלון קצר. לאחר בדיקתו תקבלו הכוונה מסודרת לגבי המסמכים הרלוונטיים.</p></details><details><summary>האם אתם מטפלים גם במחזור משכנתא?</summary><p>כן. בודקים את המשכנתא הקיימת, עלויות היציאה והחלופות כדי להבין אם יש מהלך נכון עבורכם.</p></details></div></section>

    <section className={styles.finalCta}><div data-reveal=""><span>לפני שחותמים, בודקים.</span><h2>בואו נבנה את התמונה שלכם.</h2></div><div className={styles.actions} data-reveal=""><a className={styles.darkButton} href="http://104.207.131.136:3747/check?source=perlstein">לשאלון ההתאמה <ArrowLeft /></a><a className={styles.outlineButton} href="tel:0533140442"><Phone /> 053-314-0442</a></div></section>
