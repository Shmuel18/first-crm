-- Harden convert_lead_to_case (031): if the 'case_opened' status is missing
-- (misconfigured / renamed), fail loudly instead of inserting a case with a
-- NULL status_id. ERRCODE P0001 maps to the action's generic 'unknown' branch.
...
BEGIN
  ...
  IF v_status_id IS NULL THEN
    RAISE EXCEPTION 'case_opened status not configured' USING ERRCODE = 'P0001';
  END IF;
