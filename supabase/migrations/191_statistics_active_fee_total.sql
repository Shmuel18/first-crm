-- =============================================================================
-- Migration 191: expected fee from the ACTIVE book in the statistics summary
-- =============================================================================
-- Kaufman wants "how much is supposed to come in from all future fees" — the
-- sum of the agreed fee (case_financials.fee_amount) across the ACTIVE case
-- book, mirroring active_loan_volume (which sums the requested mortgage). The
-- existing fee figures are all "executed in the period"; this is the forward
-- pipeline. Adds one field (`active_fee_total`) to the financial object — plain
-- CREATE OR REPLACE keeps the whole body otherwise. Manager-only (is_admin).
-- Dependencies: 187 (function body), 186 (case_payouts), case_financials.
-- =============================================================================
...
INSERT INTO public.schema_version (version) VALUES (191) ON CONFLICT DO NOTHING;
