-- =============================================================================
-- Migration 142: create_case_draft accepts all 4 role_in_case values
-- =============================================================================
-- The new-case draft form now lets the advisor pick the borrower's role
-- (לווה / ערב / בעל זכויות / לווה ממשכן) at creation time, not only after the
-- case is saved. The create_case_draft RPC (body last set in migration 092)
-- still hard-validated only the original two roles ('borrower','guarantor') and
-- RAISEd on the others, so a draft carrying 'rights_owner' or
-- 'mortgaging_borrower' would fail to save. The case_borrowers.role_in_case
-- CHECK was already widened to all four in migration 111; this brings the RPC's
-- own guard in line. ONLY the role-validation list changes — the rest of the
-- function is identical to migration 092.
--
-- Idempotent (CREATE OR REPLACE). Dependencies: 092 (prior body), 111 (4-role
-- column constraint).
-- =============================================================================
...
COMMENT ON FUNCTION public.create_case_draft(TEXT, JSONB) IS
  'Creates a case draft with borrowers atomically; default status is case_opened. role_in_case accepts all 4 values (migration 142).';
