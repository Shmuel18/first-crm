-- =============================================================================
-- Migration 187: NET fee in the statistics summary (F14 part B)
-- =============================================================================
-- Adds `executed_payout_total` to the financial block of get_statistics_summary
-- — the sum of case_payouts (commissions/salaries, migration 186) for the cases
-- executed in the period. The UI shows GROSS (executed_fee_total) and derives
-- NET = gross − payouts (and from net: מעשר 10% / חומש 20%, F15).
--
-- Same 3-arg signature as migration 136 — a plain CREATE OR REPLACE keeps the
-- whole body; only the financial object gains one field. Manager-only (is_admin).
-- Dependencies: 136 (function body), 186 (case_payouts).
-- =============================================================================
...
INSERT INTO public.schema_version (version) VALUES (187) ON CONFLICT DO NOTHING;
