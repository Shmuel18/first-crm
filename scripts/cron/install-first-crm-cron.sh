...
#   * Failure visibility = /var/log/first-crm-cron.log (HTTP code + body) +
#     non-zero exit. The backup-watchdog cron (06:00 UTC) checks backup
#     freshness and emails admins if there's been no successful backup in 26h
#     (SRE-2); the Settings → Integrations card shows the same status.
# =============================================================================
set -euo pipefail
...
*/5 * * * *  root /usr/local/bin/first-crm-cron.sh email-intake
0 4 * * *    root /usr/local/bin/first-crm-cron.sh time-clock-reminder
0 * * * *    root /usr/local/bin/first-crm-cron.sh ai-digest
CRONEOF
chmod 0644 "$CRONFILE"
...
echo "✓ installed $CRONFILE (UTC; backup 02:00, cleanup 03:30, sla 05:00, reminders */15m, backup-watchdog 06:00, erasure-watchdog 06:30, ai-digest hourly)"
