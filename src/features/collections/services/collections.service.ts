...
import type { CaseFeePaymentRow, CollectionOverviewRow, FeePayment } from '../types';

// Explicit column list (never select('*')) mirroring CaseFeePaymentRow.
// case_fee_payments isn't in the generated Database types yet, so the read goes
// through an untyped handle — same pattern as case-payouts / case-properties.
...
export async function listCaseFeePayments(caseId: CaseId): Promise<FeePayment[]> {
  ...
  return ((data ?? []) as unknown as CaseFeePaymentRow[]).map((r) => ({
    id: r.id,
    caseId: r.case_id,
    paidOn: r.paid_on,
...
export async function getCollectionsOverview(): Promise<CollectionOverviewRow[]> {
  ...
  const { data, error } = await db.rpc('collections_overview');
  if (error) {
    console.error('[collections] overview rpc error', { code: error.code });
    return [];
  }

  return ((data ?? []) as OverviewRpcRow[]).map((r) => ({
    caseId: r.case_id,
