...
import { resolveUploadContext } from '../services/documents.service';
...
export async function finalizeUploadAction(
  ...
  const folder = input.storagePath.substring(0, input.storagePath.lastIndexOf('/'));
  const fileName = input.storagePath.substring(input.storagePath.lastIndexOf('/') + 1);
  ...
  const { error: insertErr } = await supabase.from('documents').insert({
    ...
    status: 'new',
    metadata: { storage_path: input.storagePath },
