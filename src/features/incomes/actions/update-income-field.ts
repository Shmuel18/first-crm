...
export async function updateIncomeFieldAction(
  ...
  if (error) {
    console.error('[updateIncomeField] db error', safeDbError(error));
    return { ok: false, error: 'unknown' };
  }
  if (!updated || updated.length === 0) return { ok: false, error: 'unauthorized' };

  // No revalidatePath — the client updates the field + subtotal/grand total
  // optimistically (FE-1); revalidating re-rendered the whole case page and
  // lost scroll.
  return { ok: true };
}
