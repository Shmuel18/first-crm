...
import type { RepaymentType, TrackInput, TrackType } from '../types';
...
export function TrackEditor({ tracks, onAdd, onRemove, onUpdate }: Props) {
...
function TrackRow({ track, onRemove, onUpdate }: { track: TrackInput; onRemove: (id: string) => void; onUpdate: (id: string, patch: Partial<TrackInput>) => void }) {
  const t = useTranslations('simulators.mix.tracks');
  const linked = track.type.endsWith('_linked');
  const balloon = track.repayment === 'balloon';

  return (
    <div className="grid gap-3 rounded-lg border border-neutral-100 bg-neutral-50 p-3 lg:grid-cols-[1.3fr_1fr_0.8fr_0.8fr_1fr_0.8fr_2.5rem]">
