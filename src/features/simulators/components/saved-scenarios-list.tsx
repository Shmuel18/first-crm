      ...
      {scenarios.length === 0 ? (
        ...
        <div className="space-y-2">
          {scenarios.slice(0, 6).map((scenario) => (
            <div key={scenario.id} className="flex items-center justify-between gap-3 rounded-lg border border-neutral-100 bg-neutral-50 px-3 py-2">
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold text-neutral-950">{scenario.title}</div>
                <div className="text-xs text-neutral-500">{formatMoney(scenario.mortgage_amount)} · {scenario.scenario_tracks.length} {t('tracks')}</div>
              </div>
