...
import { useInlineMutationSync } from './use-inline-mutation-sync';
import { useOptimisticIds } from './use-optimistic-ids';
...
export function useExpenseRows(
  ...
  const [rows, setRows] = useState<CaseExpenseRow[]>(() => [...expenses]);
  ...
  if (sig !== prevSig) {
    // Always advance past the payload; APPLY only when idle and no refresh is
    // owed — a mid-mutation payload predates a write and would revert it.
    setPrevSig(sig);
    if (pendingCount === 0 && !refreshOwed) setRows([...expenses]);
  }
