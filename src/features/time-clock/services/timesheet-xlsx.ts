...
import { entryMinutes } from '../domain/hours';
...
export async function generateTimesheetXlsx(
  ...
  // Summary sheet: one row per employee with their period total.
  ...
  for (const { employee, entries } of data) {
    ...
    summary.addRow({ name: neutralize(fullName(employee)), total: hours(mins) });
  }
  ...
  for (const { employee, entries } of data) {
    ...
    for (const e of chrono) {
      sheet.addRow({
        ...
        hours: hours(entryMinutes(e, nowMs)),
        note: e.note ? neutralize(e.note) : '',
      });
    }
    const totalMins = chrono.reduce((acc, e) => acc + entryMinutes(e, nowMs), 0);
    const totalRow = sheet.addRow({ date: labels.total, hours: hours(totalMins) });
