...
export default async function CaseScenarioViewPage({
  params,
}: {
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl font-semibold text-neutral-950">{data.meta.title}</h1>
          {data.caseInfo ? <p className="mt-1 text-sm text-neutral-500">{data.caseInfo.caseNumber}</p> : null}
        </div>
        <div className="flex items-center gap-2">
          <Link
            href={`/cases/${id}/simulators/mix`}
