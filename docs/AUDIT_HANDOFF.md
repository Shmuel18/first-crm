...
| Dashboard SQL pagination — **NOT implemented** | Stale claim corrected 2026-06-01: there is no `listCasesPaged`, `parseCasePage`, or `DashboardPagination` anywhere in the codebase. The dashboard intentionally **full-fetches**: `src/app/(app)/cases/page.tsx` calls the unbounded `listCases()` (`src/features/cases/services/cases.service.ts` — its optional `limit` filter, _"unset = unbounded"_, is never passed by the dashboard), then runs search / sort / filtering client-side over the entire active-or-archive set and renders every matching row. No SQL `.range()`, no `?page=` param, no pagination UI. Cheap at ~80-case scale; revisit (move search server-side) only if the book ever grows into the many hundreds. |
