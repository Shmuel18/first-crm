...
| 2026-06-17 | 8 | R8 fix commit `024c59d0` (6 files; parallel simulator/font-tracing work EXCLUDED — staged via `:(literal)` pathspec) | Pass | `git diff --cached --name-status` = exactly mig 190 + add-empty-borrower.ts + 2 borrower routes + ROUND-08-HANDOFF + MASTER_LEDGER |
| 2026-06-17 | 8 | Mig 190 applied to **eyujzasggzjocsxakkoi** (Vultr) + **uknsayoyvffkxamofczy** (Kaufman prod), node+pg single tx, apply-before-push | Pass | Vultr 189→190; Kaufman already 190 (pre-applied) → idempotent re-assert; both: `add_empty_borrower_to_case` + `update_borrower_in_case` canonical (`_assert_can_edit_case`), incomes/obligations INSERT canonical (`can_edit_case`) — psql-verified |
